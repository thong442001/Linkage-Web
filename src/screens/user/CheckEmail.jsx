  ...
  const handleCheckOTP = async () => {
    ...
    try {
      ...
      if (response.status) {
        ...
        setTimeout(() => {
          setIsSuccess(false);
          navigate("/create-new-pass", { state: { gmail } });
        }, 2000);
      } else {
        setIsLoading(false);
        ...
        <form className="form">
          <div className="header">
            ...
            <p className="subtitle">
              Chúng tôi đã gửi mã xác nhận gồm 4 số đến email{" "}
              <span style={{ color: "#1877f2", fontWeight: "600" }}>{gmail}</span>. Nhập mã để đặt lại mật khẩu.
            </p>
          </div>
